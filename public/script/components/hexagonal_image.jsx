...
class HexagonalImage extends Component {
    ...
    handleToggle(){
    	if (this.state.nft_list.length == 0)
    ...
    async convertUrlToBase64(url, port=undefined, action=undefined){
    	...
    	if ((url == undefined) || (url == null))
			...
			port.postMessage({message: "hexagon_get_base64", publicKey: url, action: action})
			port.onMessage.addListener((msg) => {
    ...
    async handleImageSelect(previewUrl){
    	this.setState({
        loading:true
      });
    	// setter
			sessionStorage.setItem('hexa_profile_image', previewUrl);
			localStorage.setItem('hexa_profile_image', previewUrl);

			this.convertUrlToBase64(previewUrl, this.state.port, "reload")
			this.setState(prevState=> ({
          isToggleOn:!prevState.isToggleOn,
      }));
    }
       ...
       this.open_port_to_backgroundScript_retrieve_all_NFTs()
 		...
 		return (
					...
					<g>
						...
						<clipPath id="SVGID_00000000194842768478956780000016010526301968842905_">
							...
							<g transform="matrix(1 0 0 1 1.907349e-06 0)" style={{ clipPath: "url('#SVGID_00000000194842768478956780000016010526301968842905_')" }}>
								<image className="svg_overflow" width="540" height="360" onClick={this.toggleModal} xlinkHref={this.state.hexaImage}  transform="matrix(0.3726 0 0 0.3726 -27.6392 1.7906)" />
						</g>
					</g>
			...
			{this.state.isToggleOn?(
					<div className="customPopup">
						<div className="popupContent">
						  <span className="closeBtn" onClick={this.toggleModal}>x</span>
	            {this.state.nft_list.length?
	              (
								<div className="imgThumbs">
                 	{this.state.nft_list.map((nft_obj, index) => 
                   <img onClick={() => this.handleImageSelect(nft_obj.Preview_URL)} data={nft_obj.Preview_URL} key={index} src={nft_obj.base64} className="thumbImage"/>
                	)}
								</div>
								)
                :(<b>No NFTs were found in your wallet.</b>)
              }
						</div>
					</div>
				):null
			...
			{this.state.loading?
				...
				null
				//(<img className="ce_avatar_image" onClick={this.toggleModal} src={this.state.hexaImage} />)
			}
			</>
		)		
	}
